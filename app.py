...
def generate():
    try:
        ...
        brand_dupe_enabled = request.form.get("brand_dupe_enabled", "false").lower() in ["true", "1", "yes"]
        brand_name = request.form.get("brand_name", "").strip()
        brand_price = request.form.get("brand_price", "").strip()
        include_on_screen_text = request.form.get("include_on_screen_text", "false").lower() in ["true", "1", "yes"]
        ...
        # 3. User instructions
        dupe_section = ""
        if brand_dupe_enabled and brand_name:
            dupe_section = f"""
- 🏷️ BRAND DUPE & PRICE ARBITRAGE ENGINE (ACTIVE - 100% OPTIONAL):
  * Target Comparison Brand: {brand_name}
  * Competitor Brand Retail Price: {brand_price if brand_price else 'High-end retail price'}
  * MANDATORY DUPE SCRIPT REQUIREMENTS:
    - Scene 1 Spoken Hook: MUST open with an irresistible, high-converting Price Arbitrage / Dupe Challenge Hook!
    - Scene 2 Video Prompt: Include a Split-Screen / Side-by-Side comparison in the Google Flow prompt.
    - Scene 3 Fabric & Stitching Comparison: Highlight fabric feel, seams, and fit that rival the expensive version.
    - Instagram Launch Kit: Include #{brand_name.replace(' ', '')}Dupe, #DupeAlert, #MeeshoDupe.
"""
        else:
            dupe_section = """
- 🏷️ BRAND DUPE ENGINE: INACTIVE (Standard Authentic Meesho Content)
  * Strictly do NOT mention or compare with any competitor brands. Focus 100% on pure Meesho product styling and review.
"""
...
- 🗣️ VOICE QUALITY, DICTION & UNBROKEN SENTENCES MANDATE (CRITICAL):
  ...
  * AFFILIATE LAUNCH SUITE: In the launch kit, include '💬 READYMADE MANYCHAT & AUTO-DM RESPONSE TEMPLATE' (full direct message text with sizing advisory and product link) and '📲 WHATSAPP & TELEGRAM AFFILIATE DEAL CARD' (clean emoji-formatted deal message).
